...
{ }
ConsoleSystem& GetConsoleSystem() noexcept
{
...
void ConsoleSystem::ToggleConsole()
{
    if (Keys::key_dest == Keys::key_console) {
        if (Client::cls.state == ca_connected) {
            ...
            Keys::key_lines[Keys::edit_line][1] = 0;
            Keys::key_linepos = 1;
        } else
            Menu::M_Menu_Main_f();
    } else
...
{
    Keys::key_dest = Keys::key_message;
    Keys::team_message = false;
}
static void Con_MessageMode2_f()
{
    Keys::key_dest = Keys::key_message;
    Keys::team_message = true;
}

void ConsoleSystem::CheckResize()
{
    int width = (Vid::vid.width >> 3) - 2;
...
void ConsoleSystem::Init()
{
    ...
    if (debuglog_) {
        std::error_code ec;
        std::filesystem::remove((std::string(Common::com_gamedir) + "/qconsole.log").c_str(), ec);
    }
    std::fill(text_.begin(), text_.end(), ' ');
    linewidth_ = -1;
    CheckResize();
...
void ConsoleSystem::Print(std::string_view txt)
{
    ...
    if (!txt.empty() && txt[0] == 1) {
        mask = 128;
        Audio::S_LocalSound("misc/talk.wav");
        index = 1;
    ...
    while (index < txt.length()) {
        ...
        if (x_ == 0) {
            Linefeed();
            if (current_ >= 0) times_[current_ % NUM_CON_TIMES] = static_cast<float>(Host::realtime);
        }
        switch (c) {
        case '\n':
            x_ = 0;
...
void ConsoleSystem::DrawNotify()
{
    ...
    if (Keys::key_dest == Keys::key_message) {
        ...
        while (Keys::chat_buffer[x]) {
            Draw::Draw_Character((x + 5) << 3, v, Keys::chat_buffer[x]);
            x++;
        }
