...
inline void M_DrawCharacter(int cx, int line, int num) { Draw::Draw_Character(cx + ((Vid::vid.width - 320) >> 1), line, num); }
...
inline void M_DrawTransPic(int x, int y, qpic_t* pic) { Draw::Draw_TransPic(x + ((Vid::vid.width - 320) >> 1), y, pic); }
void M_DrawPic(int x, int y, qpic_t* pic) { Draw::Draw_Pic(x + ((Vid::vid.width - 320) >> 1), y, pic); }
...
inline void M_DrawTransPicTranslate(int x, int y, qpic_t* pic) { Draw::Draw_TransPicTranslate(x + ((Vid::vid.width - 320) >> 1), y, pic, translationTable.data()); }

inline void M_DrawTextBox(int x, int y, int width, int lines) {
    qpic_t* p = Draw::Draw_CachePic("gfx/box_tl.lmp"); int cx = x, cy = y; M_DrawTransPic(cx, cy, p);
    p = Draw::Draw_CachePic("gfx/box_ml.lmp"); for (int n = 0; n < lines; n++) { cy += 8; M_DrawTransPic(cx, cy, p); }
    M_DrawTransPic(cx, cy + 8, Draw::Draw_CachePic("gfx/box_bl.lmp")); cx += 8;
    while (width > 0) {
        cy = y; M_DrawTransPic(cx, cy, Draw::Draw_CachePic("gfx/box_tm.lmp"));
        for (int n = 0; n < lines; n++) { cy += 8; M_DrawTransPic(cx, cy, Draw::Draw_CachePic("gfx/box_mm2.lmp")); }
        M_DrawTransPic(cx, cy + 8, Draw::Draw_CachePic("gfx/box_bm.lmp")); width -= 2; cx += 16;
    }
    cy = y; M_DrawTransPic(cx, cy, Draw::Draw_CachePic("gfx/box_tr.lmp"));
    p = Draw::Draw_CachePic("gfx/box_mr.lmp"); for (int n = 0; n < lines; n++) { cy += 8; M_DrawTransPic(cx, cy, p); }
...
static inline bool HandleNavKeys(int key, int& cursor, int max_items, const char* snd = "misc/menu1.wav") {
    if (key == Keys::K_DOWNARROW || key == Keys::K_RIGHTARROW) { Audio::S_LocalSound(snd); cursor = (cursor + 1) % max_items; return true; }
    if (key == Keys::K_UPARROW || key == Keys::K_LEFTARROW) { Audio::S_LocalSound(snd); cursor = (cursor - 1 + max_items) % max_items; return true; }
    return false;
}

static inline void DrawMenuHeader(const char* title_pic) {
    M_DrawTransPic(16, 4, Draw::Draw_CachePic("gfx/qplaque.lmp"));
    qpic_t* p = Draw::Draw_CachePic(title_pic); M_DrawPic((320 - p->width) / 2, 4, p);
}

static inline void DrawMenuDot(int x, int y, int cursor) {
    M_DrawTransPic(x, y + cursor * 20, Draw::Draw_CachePic(Common::va("gfx/menudot%i.lmp", (int)(Host::host_time * 10) % 6 + 1)));
}
...
void M_ToggleMenu_f() {
    ...
    if (Keys::key_dest == Keys::key_menu) {
        ...
        Keys::key_dest = Keys::key_game; m_state = MenuState::None; return;
    }
    if (Keys::key_dest == Keys::key_console) Console::ConsoleSystem::ToggleConsole_f(); else M_Menu_Main_f();
}

void M_Menu_Main_f() { if (Keys::key_dest != Keys::key_menu) { m_save_demonum = Client::cls.demonum; Client::cls.demonum = -1; } Keys::key_dest = Keys::key_menu; m_state = MenuState::Main; m_entersound = true; }
void M_Main_Draw() { DrawMenuHeader("gfx/ttl_main.lmp"); M_DrawTransPic(72, 32, Draw::Draw_CachePic("gfx/mainmenu.lmp")); DrawMenuDot(54, 32, m_main_cursor); }
void M_Main_Key(int key) {
    if (key == Keys::K_ESCAPE) { Keys::key_dest = Keys::key_game; m_state = MenuState::None; Client::cls.demonum = m_save_demonum; if (Client::cls.demonum != -1 && !Client::cls.demoplayback && Client::cls.state != ca_connected) Client::CL_NextDemo(); return; }
...
void M_Menu_SinglePlayer_f() { Keys::key_dest = Keys::key_menu; m_state = MenuState::SinglePlayer; m_entersound = true; }
void M_SinglePlayer_Draw() { DrawMenuHeader("gfx/ttl_sgl.lmp"); M_DrawTransPic(72, 32, Draw::Draw_CachePic("gfx/sp_menu.lmp")); DrawMenuDot(54, 32, m_singleplayer_cursor); }
void M_SinglePlayer_Key(int key) {
    if (key == Keys::K_ESCAPE) { M_Menu_Main_f(); return; }
    if (HandleNavKeys(key, m_singleplayer_cursor, 3)) return;
    if (key == Keys::K_ENTER) {
        m_entersound = true;
        switch (m_singleplayer_cursor) {
        case 0:
            if (Server::sv.active && !Screen::GetScreenSystem().ModalMessage("Are you sure you want to\nstart a new game?\n")) break;
            Keys::key_dest = Keys::key_game; if (Server::sv.active) Cmd::BufferAddText("disconnect\n");
...
void M_ScanSaves() {
    for (int i = 0; i < MAX_SAVEGAMES; i++) {
        ...
        char name[MAX_OSPATH]; sprintf_s(name, sizeof(name), "%s/s%i.sav", Common::com_gamedir, i);
...
void M_Menu_Load_f() { m_entersound = true; m_state = MenuState::Load; Keys::key_dest = Keys::key_menu; M_ScanSaves(); }
void M_Menu_Save_f() { if (!Server::sv.active || Client::cl.intermission || Server::svs.maxclients != 1) return; m_entersound = true; m_state = MenuState::Save; Keys::key_dest = Keys::key_menu; M_ScanSaves(); }

static inline void DrawSaveLoadCommon(const char* pic) {
    qpic_t* p = Draw::Draw_CachePic(pic); M_DrawPic((320 - p->width) / 2, 4, p);
...
void M_Load_Key(int k) {
    if (k == Keys::K_ESCAPE) { M_Menu_SinglePlayer_f(); return; }
    if (HandleNavKeys(k, load_cursor, MAX_SAVEGAMES)) return;
    if (k == Keys::K_ENTER) {
        Audio::S_LocalSound("misc/menu2.wav"); if (!loadable[load_cursor]) return;
        m_state = MenuState::None; Keys::key_dest = Keys::key_game; Screen::GetScreenSystem().BeginLoadingPlaque();
...
void M_Save_Key(int k) {
    if (k == Keys::K_ESCAPE) { M_Menu_SinglePlayer_f(); return; }
    if (HandleNavKeys(k, load_cursor, MAX_SAVEGAMES)) return;
    if (k == Keys::K_ENTER) { m_state = MenuState::None; Keys::key_dest = Keys::key_game; Cmd::BufferAddText(Common::va("save s%i\n", load_cursor)); }
}

void M_Menu_MultiPlayer_f() { Keys::key_dest = Keys::key_menu; m_state = MenuState::MultiPlayer; m_entersound = true; }
void M_MultiPlayer_Draw() {
    DrawMenuHeader("gfx/p_multi.lmp"); M_DrawTransPic(72, 32, Draw::Draw_CachePic("gfx/mp_menu.lmp")); DrawMenuDot(54, 32, m_multiplayer_cursor);
...
void M_MultiPlayer_Key(int key) {
    ...
    if (key == Keys::K_ENTER) {
        ...
        else if (Net::tcpipAvailable) M_Menu_LanConfig_f();
    }
}
...
void M_Menu_Setup_f() {
    Keys::key_dest = Keys::key_menu; m_state = MenuState::Setup; m_entersound = true;
    setup_myname = Client::cl_name.string; setup_hostname = Net::hostname.string;
...
void M_Setup_Draw() {
    ...
    if (setup_cursor == 0) M_DrawCharacter(168 + 8 * (int)setup_hostname.length(), setup_cursor_table[0], 10 + ((int)(Host::realtime * 4) & 1));
    if (setup_cursor == 1) M_DrawCharacter(168 + 8 * (int)setup_myname.length(), setup_cursor_table[1], 10 + ((int)(Host::realtime * 4) & 1));
}

void M_Setup_Key(int k) {
    if (k == Keys::K_ESCAPE) { M_Menu_MultiPlayer_f(); return; }
    if (k == Keys::K_UPARROW)   { Audio::S_LocalSound("misc/menu1.wav"); setup_cursor = (setup_cursor - 1 + 5) % 5; return; }
    if (k == Keys::K_DOWNARROW) { Audio::S_LocalSound("misc/menu1.wav"); setup_cursor = (setup_cursor + 1) % 5; return; }
    auto AdjColor = [](int dir, int& color) { Audio::S_LocalSound("misc/menu3.wav"); color = (color + dir + 14) % 14; };
    if (k == Keys::K_LEFTARROW) { if (setup_cursor == 2) AdjColor(-1, setup_top); if (setup_cursor == 3) AdjColor(-1, setup_bottom); return; }
    if (k == Keys::K_RIGHTARROW) { if (setup_cursor == 2) AdjColor(1, setup_top); if (setup_cursor == 3) AdjColor(1, setup_bottom); return; }
    if (k == Keys::K_ENTER) {
        if (setup_cursor == 2) AdjColor(1, setup_top);
        else if (setup_cursor == 3) AdjColor(1, setup_bottom);
        else if (setup_cursor == 4) {
            if (Client::cl_name.string != setup_myname) Cmd::BufferAddText(Common::va("name \"%s\"\n", setup_myname.c_str()));
            if (Net::hostname.string != setup_hostname) Cvar::Set("hostname", setup_hostname.c_str());
            if (setup_top != setup_oldtop || setup_bottom != setup_oldbottom) Cmd::BufferAddText(Common::va("color %i %i\n", setup_top, setup_bottom));
...
void M_Menu_Options_f() { Keys::key_dest = Keys::key_menu; m_state = MenuState::Options; m_entersound = true; }
...
void M_AdjustSliders(int dir) {
    ...
    case 4: Cvar::SetValue("gamma", std::clamp<float>(View::v_gamma.value - static_cast<float>(dir * 0.05), 0.5f, 1.0f)); break;
    case 5: Cvar::SetValue("sensitivity", std::clamp<float>(Client::sensitivity.value + static_cast<float>(dir * 0.5), 1.0f, 11.0f)); break;
    case 6: Cvar::SetValue("bgmvolume", std::clamp<float>(Audio::bgmvolume.value + static_cast<float>(dir * 0.1), 0.0f, 1.0f)); break;
    case 7: Cvar::SetValue("volume", std::clamp<float>(Audio::volume.value + static_cast<float>(dir * 0.1), 0.0f, 1.0f)); break;
    case 8: Cvar::SetValue("cl_forwardspeed", (Client::cl_forwardspeed.value > 200) ? 200.0f : 400.0f); Cvar::SetValue("cl_backspeed", (Client::cl_forwardspeed.value > 200) ? 200.0f : 400.0f); break;
    case 9: Cvar::SetValue("m_pitch", -Client::m_pitch.value); break;
    case 10: Cvar::SetValue("lookspring", static_cast<float>(!Client::lookspring.value)); break;
    case 11: Cvar::SetValue("lookstrafe", static_cast<float>(!Client::lookstrafe.value)); break;
    }
}
...
void M_Options_Draw() {
    ...
    M_Print(16, 64, "            Brightness"); M_DrawSlider(220, 64, static_cast<float>((1.0 - View::v_gamma.value) / 0.5));
    M_Print(16, 72, "           Mouse Speed"); M_DrawSlider(220, 72, (Client::sensitivity.value - 1) / 10);
    M_Print(16, 80, "       CD Music Volume"); M_DrawSlider(220, 80, Audio::bgmvolume.value);
    M_Print(16, 88, "          Sound Volume"); M_DrawSlider(220, 88, Audio::volume.value);
    M_Print(16, 96, "            Always Run"); M_DrawCheckbox(220, 96, Client::cl_forwardspeed.value > 200);
    M_Print(16, 104, "          Invert Mouse"); M_DrawCheckbox(220, 104, Client::m_pitch.value < 0);
    M_Print(16, 112, "            Lookspring"); M_DrawCheckbox(220, 112, static_cast<int>(Client::lookspring.value));
    M_Print(16, 120, "            Lookstrafe"); M_DrawCheckbox(220, 120, static_cast<int>(Client::lookstrafe.value));
    DrawLineCursor(200, 32, options_cursor);
}

void M_Options_Key(int k) {
    if (k == Keys::K_ESCAPE) { M_Menu_Main_f(); return; }
    if (k == Keys::K_ENTER) {
        m_entersound = true;
        switch (options_cursor) {
        case 0: M_Menu_Keys_f(); break; case 1: m_state = MenuState::None; Console::ConsoleSystem::ToggleConsole_f(); break;
        case 2: Cmd::BufferAddText("exec default.cfg\n"); break;
        default: M_AdjustSliders(1); break;
        }
        return;
    }
    if (k == Keys::K_UPARROW)   { Audio::S_LocalSound("misc/menu1.wav"); options_cursor = (options_cursor - 1 + OPTIONS_ITEMS) % OPTIONS_ITEMS; }
    if (k == Keys::K_DOWNARROW) { Audio::S_LocalSound("misc/menu1.wav"); options_cursor = (options_cursor + 1) % OPTIONS_ITEMS; }
...
void M_Menu_Keys_f() { Keys::key_dest = Keys::key_menu; m_state = MenuState::Keys; m_entersound = true; }

void M_FindKeysForCommand(std::string_view command, std::array<int, 2>& twokeys) {
    twokeys[0] = twokeys[1] = -1; int count = 0;
    for (int j = 0; j < 256; j++) {
        if (!Keys::keybindings[j].empty() && Keys::keybindings[j] == command) { twokeys[count++] = j; if (count == 2) break; }
    }
}

void M_UnbindCommand(std::string_view command) {
    for (int j = 0; j < 256; j++) { if (!Keys::keybindings[j].empty() && Keys::keybindings[j] == command) Keys::Key_SetBinding(j, ""); }
}

void M_Keys_Draw() {
    qpic_t* p = Draw::Draw_CachePic("gfx/ttl_cstm.lmp"); M_DrawPic((320 - p->width) / 2, 4, p);
    ...
    for (size_t i = 0; i < bindnames.size(); i++) {
        ...
        if (keys[0] == -1) M_Print(140, y, "???");
            ...
            const char* name = Keys::Key_KeynumToString(keys[0]); M_Print(140, y, name);
            if (keys[1] != -1) { int x = static_cast<int>(strlen(name)) * 8; M_Print(140 + x + 8, y, "or"); M_Print(140 + x + 32, y, Keys::Key_KeynumToString(keys[1])); }
        }
    }
...
void M_Keys_Key(int k) {
    ...
    if (bind_grab) {
        ...
        if (k != Keys::K_ESCAPE && k != '`') Cmd::BufferInsertText(Common::va("bind \"%s\" \"%.70s\"\n", Keys::Key_KeynumToString(k), bindnames[keys_cursor].command.data()));
        bind_grab = false; return;
    }
    if (k == Keys::K_ESCAPE) { M_Menu_Options_f(); return; }
    if (HandleNavKeys(k, keys_cursor, static_cast<int>(bindnames.size()))) return;
    if (k == Keys::K_ENTER) {
        M_FindKeysForCommand(bindnames[keys_cursor].command, keys); Audio::S_LocalSound("misc/menu2.wav");
        if (keys[1] != -1) M_UnbindCommand(bindnames[keys_cursor].command);
        bind_grab = true;
    } else if (k == Keys::K_BACKSPACE || k == Keys::K_DEL) { Audio::S_LocalSound("misc/menu2.wav"); M_UnbindCommand(bindnames[keys_cursor].command); }
}

constexpr int NUM_HELP_PAGES = 6;
void M_Menu_Help_f() { Keys::key_dest = Keys::key_menu; m_state = MenuState::Help; m_entersound = true; help_page = 0; }
void M_Help_Draw() { M_DrawPic(0, 0, Draw::Draw_CachePic(Common::va("gfx/help%i.lmp", help_page))); }
void M_Help_Key(int key) {
    if (key == Keys::K_ESCAPE) { M_Menu_Main_f(); return; }
    if (key == Keys::K_UPARROW || key == Keys::K_RIGHTARROW) { m_entersound = true; help_page = (help_page + 1) % NUM_HELP_PAGES; }
    if (key == Keys::K_DOWNARROW || key == Keys::K_LEFTARROW) { m_entersound = true; help_page = (help_page - 1 + NUM_HELP_PAGES) % NUM_HELP_PAGES; }
}

void M_Menu_Quit_f() { Keys::key_dest = Keys::key_console; Host::Host_Quit_f(); }

constexpr auto lanConfig_cursor_table = std::array{ 72, 92, 124 };
constexpr int NUM_LANCONFIG_CMDS = 3;

void M_Menu_LanConfig_f() {
    Keys::key_dest = Keys::key_menu; m_state = MenuState::LanConfig; m_entersound = true;
    if (lanConfig_cursor == -1) lanConfig_cursor = JoiningGame() ? 2 : 1;
    if (StartingGame() && lanConfig_cursor == 2) lanConfig_cursor = 1;
    lanConfig_port = Net::DEFAULTnet_hostport; lanConfig_portname = std::to_string(lanConfig_port);
    m_return_onerror = false; m_return_reason.clear();
}

void M_LanConfig_Draw() {
    DrawMenuHeader("gfx/p_multi.lmp"); int basex = (320 - Draw::Draw_CachePic("gfx/p_multi.lmp")->width) / 2;
    M_Print(basex, 32, Common::va("%s - TCP/IP", StartingGame() ? "New Game" : "Join Game")); basex += 8;
    M_Print(basex, 52, "Address:"); M_Print(basex + 9 * 8, 52, Net::my_tcpip_address);
    ...
    if (lanConfig_cursor == 0) M_DrawCharacter(basex + 9 * 8 + 8 * (int)lanConfig_portname.length(), lanConfig_cursor_table[0], 10 + ((int)(Host::realtime * 4) & 1));
    if (lanConfig_cursor == 2) M_DrawCharacter(basex + 16 + 8 * (int)lanConfig_joinname.length(), lanConfig_cursor_table[2], 10 + ((int)(Host::realtime * 4) & 1));
    if (!m_return_reason.empty()) M_PrintWhite(basex, 148, m_return_reason.c_str());
}

void M_LanConfig_Key(int key) {
    if (key == Keys::K_ESCAPE) { M_Menu_MultiPlayer_f(); return; }
    if (key == Keys::K_UPARROW)   { Audio::S_LocalSound("misc/menu1.wav"); lanConfig_cursor = (lanConfig_cursor - 1 + NUM_LANCONFIG_CMDS) % NUM_LANCONFIG_CMDS; }
    if (key == Keys::K_DOWNARROW) { Audio::S_LocalSound("misc/menu1.wav"); lanConfig_cursor = (lanConfig_cursor + 1) % NUM_LANCONFIG_CMDS; }
    if (key == Keys::K_ENTER) {
        ...
        if (lanConfig_cursor == 2) {
            m_return_state = m_state; m_return_onerror = true; Keys::key_dest = Keys::key_game; m_state = MenuState::None;
    ...
    int l = Common::Q_atoi(lanConfig_portname.c_str()); if (l <= 65535) lanConfig_port = l;
...
void M_Menu_GameOptions_f() {
    Keys::key_dest = Keys::key_menu; m_state = MenuState::GameOptions; m_entersound = true;
...
void M_GameOptions_Draw() {
    ...
    M_Print(0, 56, "      Max players"); M_Print(160, 56, Common::va("%i", maxplayers));
    M_Print(0, 64, "        Game Type"); M_Print(160, 64, Server::coop.value ? "Cooperative" : "Deathmatch");
    ...
    if (Common::rogue) {
        ...
        int idx = static_cast<int>(Server::teamplay.value); if (idx >= 1 && idx <= 6) team_msg = rmsgs[idx];
    } else {
        if ((int)Server::teamplay.value == 1) team_msg = "No Friendly Fire";
        else if ((int)Server::teamplay.value == 2) team_msg = "Friendly Fire";
    }
    ...
    M_Print(0, 88, "       Frag Limit"); M_Print(160, 88, (Server::fraglimit.value == 0) ? "none" : Common::va("%i frags", (int)Server::fraglimit.value));
    M_Print(0, 96, "       Time Limit"); M_Print(160, 96, (Server::timelimit.value == 0) ? "none" : Common::va("%i minutes", (int)Server::timelimit.value));
    M_Print(0, 112, "         Episode");
    const episode_t* ep_ptr = episodes.data(); const level_t* lvl_ptr = levels.data();
    if (Common::hipnotic) { ep_ptr = hipnoticepisodes.data(); lvl_ptr = hipnoticlevels.data(); }
    else if (Common::rogue) { ep_ptr = rogueepisodes.data(); lvl_ptr = roguelevels.data(); }
...
void M_NetStart_Change(int dir) {
    ...
    case 1:
        maxplayers += dir;
        if (maxplayers > Server::svs.maxclientslimit) { maxplayers = Server::svs.maxclientslimit; m_serverInfoMessage = true; m_serverInfoMessageTime = Host::realtime; }
        if (maxplayers < 2) maxplayers = 2;
        break;
    case 2: Cvar::SetValue("coop", static_cast<float>(Server::coop.value ? 0 : 1)); break;
    case 3: {
        int count = Common::rogue ? 6 : 2; float new_val = Server::teamplay.value + dir;
        if (new_val > count) new_val = 0; else if (new_val < 0) new_val = static_cast<float>(count);
        Cvar::SetValue("teamplay", new_val); break;
    }
    case 4: { float new_sk = Server::skill.value + dir; if (new_sk > 3) new_sk = 0; else if (new_sk < 0) new_sk = 3; Cvar::SetValue("skill", new_sk); break; }
    case 5: { float new_fl = Server::fraglimit.value + dir * 10; if (new_fl > 100) new_fl = 0; else if (new_fl < 0) new_fl = 100; Cvar::SetValue("fraglimit", new_fl); break; }
    case 6: { float new_tl = Server::timelimit.value + dir * 5; if (new_tl > 60) new_tl = 0; else if (new_tl < 0) new_tl = 60; Cvar::SetValue("timelimit", new_tl); break; }
    case 7: { int count = Common::hipnotic ? 6 : (Common::rogue ? 4 : (registered.value ? 7 : 2)); startepisode = (startepisode + dir + count) % count; startlevel = 0; break; }
    case 8: { int count = Common::hipnotic ? hipnoticepisodes[startepisode].levels : (Common::rogue ? rogueepisodes[startepisode].levels : episodes[startepisode].levels); startlevel = (startlevel + dir + count) % count; break; }
    }
}

void M_GameOptions_Key(int key) {
    if (key == Keys::K_ESCAPE) { M_Menu_LanConfig_f(); return; }
    if (key == Keys::K_UPARROW)   { Audio::S_LocalSound("misc/menu1.wav"); gameoptions_cursor = (gameoptions_cursor - 1 + NUM_GAMEOPTIONS) % NUM_GAMEOPTIONS; }
    if (key == Keys::K_DOWNARROW) { Audio::S_LocalSound("misc/menu1.wav"); gameoptions_cursor = (gameoptions_cursor + 1) % NUM_GAMEOPTIONS; }
    if (key == Keys::K_LEFTARROW && gameoptions_cursor > 0)  { Audio::S_LocalSound("misc/menu3.wav"); M_NetStart_Change(-1); }
    if (key == Keys::K_RIGHTARROW && gameoptions_cursor > 0) { Audio::S_LocalSound("misc/menu3.wav"); M_NetStart_Change(1); }
    if (key == Keys::K_ENTER) {
...
void M_Menu_Search_f() { Keys::key_dest = Keys::key_menu; m_state = MenuState::Search; m_entersound = false; Net::slistSilent = true; Net::slistLocal = false; searchComplete = false; Net::NET_Slist_f(); }
void M_Search_Draw() {
    qpic_t* p = Draw::Draw_CachePic("gfx/p_multi.lmp"); M_DrawPic((320 - p->width) / 2, 4, p);
    int x = (320 / 2) - 44; M_DrawTextBox(x - 8, 32, 12, 1); M_Print(x, 40, "Searching...");
    if (Net::slistInProgress) { Net::NET_Poll(); return; }
    if (!searchComplete) { searchComplete = true; searchCompleteTime = Host::realtime; }
    if (Net::hostCacheCount) { M_Menu_ServerList_f(); return; }
    M_PrintWhite((320 - 22 * 8) / 2, 64, "No Quake servers found");
    if ((Host::realtime - searchCompleteTime) >= 3.0) M_Menu_LanConfig_f();
}
void M_Search_Key() {}

void M_Menu_ServerList_f() { Keys::key_dest = Keys::key_menu; m_state = MenuState::SList; m_entersound = true; slist_cursor = 0; m_return_onerror = false; m_return_reason.clear(); slist_sorted = false; }
void M_ServerList_Draw() {
    if (!slist_sorted && Net::hostCacheCount > 1) {
        std::sort(Net::hostcache.begin(), Net::hostcache.begin() + Net::hostCacheCount, [](const Net::hostcache_t& a, const Net::hostcache_t& b) { return strcmp(a.name, b.name) < 0; });
        slist_sorted = true;
    }
    qpic_t* p = Draw::Draw_CachePic("gfx/p_multi.lmp"); M_DrawPic((320 - p->width) / 2, 4, p);
    for (int n = 0; n < Net::hostCacheCount; n++) {
        char string[64];
        if (Net::hostcache[n].maxusers) sprintf_s(string, sizeof(string), "%-15.15s %-15.15s %2u/%2u\n", Net::hostcache[n].name, Net::hostcache[n].map, Net::hostcache[n].users, Net::hostcache[n].maxusers);
...
void M_ServerList_Key(int k) {
    if (k == Keys::K_ESCAPE) { M_Menu_LanConfig_f(); return; }
    if (k == Keys::K_SPACE)  { M_Menu_Search_f(); return; }
    if (HandleNavKeys(k, slist_cursor, Net::hostCacheCount)) return;
    if (k == Keys::K_ENTER) {
        Audio::S_LocalSound("misc/menu2.wav"); m_return_state = m_state; m_return_onerror = true; slist_sorted = false;
        Keys::key_dest = Keys::key_game; m_state = MenuState::None; Cmd::BufferAddText(Common::va("connect \"%s\"\n", Net::hostcache[slist_cursor].cname));
    }
}
...
void M_Draw() {
    ...
    if (m_entersound) { Audio::S_LocalSound("misc/menu2.wav"); m_entersound = false; }
}
