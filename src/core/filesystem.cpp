...
const char* COM_Parse(const char* data)
{
    ...
    while (true) {
        c = *data;
        if (c == '\0' || static_cast<unsigned char>(c) <= ' ' || c == '{' || c == '}' || c == ')' || c == '('
            || c == '\'' || c == ':')
            break;
        if (len < static_cast<int>(sizeof(com_token)) - 1) com_token[len++] = c;
        data++;
                ...
                if (std::strcmp(entry.name, filename) == 0) {
                    ...
                    if (handle) {
                        *handle = pak->handle;
                        ...
                        if (*file) fseek(*file, entry.filepos, SEEK_SET);
                    }
...
void COM_InitFilesystem(const char* basedir)
{
    ...
    if (i && i < com_argc - 1)
        strcpy_s(basedir_buf, sizeof(basedir_buf), com_argv[i + 1]);
    else
        strcpy_s(basedir_buf, sizeof(basedir_buf), basedir);
    ...
    for (uint32_t i = 0; i < 256; ++i) {
        ...
        for (int j = 0; j < 8; ++j) {
            if ((value ^ temp) & 0x8000)
                value = static_cast<std::uint16_t>((value << 1) ^ 0x1021);
            else
