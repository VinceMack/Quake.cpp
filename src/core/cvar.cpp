...
void CvarRegistry::Set(std::string_view var_name, std::string_view value) {
    cvar_t* var = FindVar(var_name);
    if (!var) {
        Console::Con_Printf("Cvar::Set: variable %.*s not found\n", static_cast<int>(var_name.length()), var_name.data());
        return;
    }
...
cvar_t* FindVar(std::string_view var_name) { return GetCvarRegistry().FindVar(var_name); }
float VariableValue(std::string_view var_name) { return GetCvarRegistry().VariableValue(var_name); }
std::string_view VariableString(std::string_view var_name) { return GetCvarRegistry().VariableString(var_name); }
std::string_view CompleteVariable(std::string_view partial) { return GetCvarRegistry().CompleteVariable(partial); }
void Set(std::string_view var_name, std::string_view value) { GetCvarRegistry().Set(var_name, value); }
void SetValue(std::string_view var_name, float value) { GetCvarRegistry().SetValue(var_name, value); }
