...
//=============================================================================
// 1. EASTL Default Memory Allocators
//=============================================================================
...
short ShortSwap(short l) { uint16_t u = static_cast<uint16_t>(l); return static_cast<short>((u >> 8) | (u << 8)); }
...
void MSG_WriteChar(sizebuf_t* sb, int c) { static_cast<byte*>(SZ_GetSpace(sb, 1))[0] = static_cast<byte>(c); }
void MSG_WriteByte(sizebuf_t* sb, int c) { static_cast<byte*>(SZ_GetSpace(sb, 1))[0] = static_cast<byte>(c); }
void MSG_WriteShort(sizebuf_t* sb, int c) {
    auto* b = static_cast<byte*>(SZ_GetSpace(sb, 2));
    b[0] = static_cast<byte>(c & 0xff); b[1] = static_cast<byte>((c >> 8) & 0xff);
}
void MSG_WriteLong(sizebuf_t* sb, int c) {
    auto* b = static_cast<byte*>(SZ_GetSpace(sb, 4));
    b[0] = static_cast<byte>(c & 0xff); b[1] = static_cast<byte>((c >> 8) & 0xff);
    b[2] = static_cast<byte>((c >> 16) & 0xff); b[3] = static_cast<byte>((c >> 24) & 0xff);
}
...
int MSG_ReadChar(void) {
    ...
    return static_cast<int8_t>(net_message.data[msg_readcount++]);
}
...
int MSG_ReadShort(void) {
    if (msg_readcount + 2 > net_message.cursize) { msg_badread = true; return -1; }
    int c = static_cast<int16_t>(net_message.data[msg_readcount] | (net_message.data[msg_readcount + 1] << 8));
    msg_readcount += 2;
    return c;
}
int MSG_ReadLong(void) {
    if (msg_readcount + 4 > net_message.cursize) { msg_badread = true; return -1; }
    int c = static_cast<int>(net_message.data[msg_readcount] | (net_message.data[msg_readcount + 1] << 8) |
                            (net_message.data[msg_readcount + 2] << 16) | (net_message.data[msg_readcount + 3] << 24));
    msg_readcount += 4;
    return c;
}
float MSG_ReadFloat(void) {
    if (msg_readcount + 4 > net_message.cursize) { msg_badread = true; return -1.0f; }
    uint32_t val = 0;
    std::memcpy(&val, &net_message.data[msg_readcount], sizeof(val));
    msg_readcount += 4;
    return std::bit_cast<float>(static_cast<uint32_t>(LittleLong(static_cast<int>(val))));
}
char* MSG_ReadString(void) {
    static char string[2048]; int l = 0;
    do {
        int c = MSG_ReadChar();
        if (c == -1 || c == 0) break;
...
void SZ_Print(sizebuf_t* buf, const char* data) {
    ...
    if (buf->data[buf->cursize - 1]) Q_memcpy(SZ_GetSpace(buf, len), data, len);
    else Q_memcpy(static_cast<byte*>(SZ_GetSpace(buf, len - 1)) - 1, data, len);
}
...
void COM_CheckRegistered(void) {
    ...
    static_registered = 1; Con_Printf("Playing registered version.\n");
}
...
byte* COM_LoadFile(const char* path, HunkType usehunk) {
    ...
    buf[len] = 0;
    Draw_BeginDisc(); Sys_FileRead(h, buf, len); COM_CloseFile(h); Draw_EndDisc();
    return buf;
}
...
void COM_InitFilesystem(void) {
    ...
    else strcpy_s(basedir_buf, sizeof(basedir_buf), host_parms.basedir);
    ...
    } else if (host_parms.cachedir) strcpy_s(com_cachedir, sizeof(com_cachedir), host_parms.cachedir);
...
void* Hunk_HighAllocName(int size, const char* name) {
    ...
    size = sizeof(hunk_t) + ((size + 15) & ~15);
    if (hunk_size - hunk_low_used - hunk_high_used < size) { Con_Printf("Hunk_HighAlloc: failed on %i bytes\n", size); return nullptr; }
    hunk_high_used += size;
...
void Cache_Report(void) { Con_DPrintf("%4.1f megabyte data cache\n", (hunk_size - hunk_high_used - hunk_low_used) / (float)(1024 * 1024)); }
...
float CvarRegistry::VariableValue(eastl::string_view var_name) {
    ...
    return var ? Q_atof(var->string.c_str()) : 0.0f;
}

eastl::string_view CvarRegistry::VariableString(eastl::string_view var_name) {
    cvar_t* var = FindVar(var_name);
    return var ? eastl::string_view(var->string.data(), var->string.length()) : "";
}
...
eastl::string_view CvarRegistry::CompleteVariable(eastl::string_view partial) {
    ...
    for (cvar_t* var = state_.vars; var; var = var->next) {
        eastl::string_view vn(var->name.data(), var->name.length());
...
void CvarRegistry::Set(eastl::string_view var_name, eastl::string_view value) {
    cvar_t* var = FindVar(var_name);
    if (!var) { Con_Printf("Cvar::Set: variable %.*s not found\n", static_cast<int>(var_name.length()), var_name.data()); return; }
    ...
    if (var->server && changed && sv.active) SV_BroadcastPrintf("\"%s\" changed to \"%s\"\n", var->name.c_str(), var->string.c_str());
}

void CvarRegistry::SetValue(eastl::string_view var_name, float value) {
    char val[32]; std::snprintf(val, sizeof(val), "%f", value); Set(var_name, val);
}

void CvarRegistry::Register(cvar_t* variable) {
    if (!variable) return;
    eastl::string_view name_view(variable->name.data(), variable->name.length());
    if (FindVar(name_view)) { Con_Printf("Can't register variable %s, allready defined\n", variable->name.c_str()); return; }
    if (Cmd::Exists(name_view)) { Con_Printf("Cvar::Register: %s is a command\n", variable->name.c_str()); return; }
    variable->value = Q_atof(variable->string.c_str());
    variable->next = state_.vars; state_.vars = variable;
    vars_map_.insert(eastl::make_pair(name_view, variable));
}

bool CvarRegistry::Command() {
    cvar_t* v = FindVar(Cmd::Argv(0));
    if (!v) return false;
    if (Cmd::Argc() == 1) { Con_Printf("\"%s\" is \"%s\"\n", v->name.c_str(), v->string.c_str()); return true; }
...
void CommandRegistry::BufferInit(void) { cmd_text_.clear(); cmd_text_.reserve(8192); }

void CommandRegistry::BufferAddText(eastl::string_view text) {
    if (cmd_text_.length() + text.length() >= 8192) { Con_Printf("Cmd::BufferAddText: overflow\n"); return; }
...
void CommandRegistry::BufferExecute(void) {
    while (!cmd_text_.empty()) {
        ...
        if (cmd_wait_) { cmd_wait_ = false; break; }
    }
}

static void StuffCmds_f(void) {
    if (Cmd::Argc() != 1) { Con_Printf("stuffcmds : execute command line parameters\n"); return; }
    eastl::string text;
    for (int i = 1; i < com_argc; i++) {
    ...
    eastl::string build; size_t i = 0;
    while (i < text.length()) {
        if (text[i] == '+') {
            i++; size_t j = i;
            while (j < text.length() && text[j] != '+' && text[j] != '-') j++;
            build += text.substr(i, j - i); build += "\n"; i = j;
        } else i++;
    }
    if (!build.empty()) Cmd::BufferInsertText(eastl::string_view(build.data(), build.length()));
}

static void Exec_f(void) {
    if (Cmd::Argc() != 2) { Con_Printf("exec <filename> : execute a script file\n"); return; }
    int mark = Hunk_LowMark();
    eastl::string_view filename = Cmd::Argv(1);
    eastl::string filename_str(filename.data(), filename.length());
    auto* f = reinterpret_cast<char*>(COM_LoadHunkFile(filename_str.c_str()));
    if (!f) { Con_Printf("couldn't exec %s\n", filename_str.c_str()); return; }
    Con_Printf("execing %s\n", filename_str.c_str());
    Cmd::BufferInsertText(f); Hunk_FreeToLowMark(mark);
}
...
static void Alias_f(void) {
    ...
    if (Cmd::Argc() == 1) {
        ...
        for (const auto& [name, value] : registry.GetAliases()) Con_Printf("%s : %s\n", name.c_str(), value.c_str());
        return;
    }
    eastl::string_view alias_name = Cmd::Argv(1);
    if (alias_name.length() >= 32) { Con_Printf("Alias name is too long\n"); return; }
    eastl::string cmd;
    for (int i = 2; i < Cmd::Argc(); ++i) {
        eastl::string_view arg = Cmd::Argv(i);
        cmd.append(arg.data(), arg.length()); cmd += " ";
    }
    cmd += "\n";
...
void CommandRegistry::Init(void) {
    AddCommand("stuffcmds", StuffCmds_f); AddCommand("exec", Exec_f);
    AddCommand("echo", Echo_f); AddCommand("alias", Alias_f);
    AddCommand("cmd", ForwardToServer); AddCommand("wait", Wait_f);
}
...
void CommandRegistry::AddCommand(eastl::string_view cmd_name, xcommand_t function) {
    ...
    if (Cvar::FindVar(cmd_name) != nullptr) { Con_Printf("Cmd::AddCommand: %.*s already defined as a var\n", static_cast<int>(cmd_name.length()), cmd_name.data()); return; }
    if (Exists(cmd_name)) { Con_Printf("Cmd::AddCommand: %.*s already defined\n", static_cast<int>(cmd_name.length()), cmd_name.data()); return; }
    commands_.emplace(eastl::string(cmd_name.data(), cmd_name.length()), std::move(function));
}

bool CommandRegistry::Exists(eastl::string_view cmd_name) { return commands_.count(cmd_name) > 0; }
...
eastl::string_view CommandRegistry::CompleteCommand(eastl::string_view partial) {
    ...
    if (it != commands_.end()) {
        ...
        if (cmd_name.length() >= partial.length()) {
            ...
            if (Q_strcasecmp(eastl::string(prefix.data(), prefix.length()).c_str(), eastl::string(partial.data(), partial.length()).c_str()) == 0) return cmd_name;
        }
    }
    return "";
}
...
eastl::string_view CommandRegistry::Args(void) { return cmd_args_; }

void CommandRegistry::TokenizeString(eastl::string_view text) {
    cmd_argv_.clear(); cmd_args_ = "";
    if (text.empty()) return;
    const char* ptr = text.data(); bool command_parsed = false;
    while (true) {
        while (*ptr && *ptr <= ' ' && *ptr != '\n') ptr++;
        if (*ptr == '\n') { ptr++; break; }
        if (!*ptr) return;
        if (command_parsed && cmd_args_.empty()) cmd_args_ = eastl::string_view(ptr);
        const char* next_ptr = COM_Parse(ptr);
        if (!next_ptr) return;
        ptr = next_ptr;
        if (!command_parsed) command_parsed = true;
        if (cmd_argv_.size() < 80) cmd_argv_.push_back(com_token);
    }
}

void CommandRegistry::ExecuteString(eastl::string_view text, Source src) {
    state_.source = src; TokenizeString(text);
    if (cmd_argv_.empty()) return;
    const auto& cmd_name = cmd_argv_[0];
    auto cmd_it = commands_.find(cmd_name);
    if (cmd_it != commands_.end()) { cmd_it->second(); return; }
    auto alias_it = aliases_.find(cmd_name);
    if (alias_it != aliases_.end()) { BufferInsertText(eastl::string_view(alias_it->second.data(), alias_it->second.length())); return; }
    if (!Cvar::Command()) Con_Printf("Unknown command \"%s\"\n", cmd_name.c_str());
}
...
void ForwardToServer(void) {
    ...
    if (Q_strcasecmp(argv0.c_str(), "cmd") != 0) {
        SZ_Print(&cls.message, argv0.c_str()); SZ_Print(&cls.message, " ");
    }
    if (Argc() > 1) {
...
std::pair<int, int> FloorDivMod(double numer, double denom) {
    ...
    if (numer >= 0.0) {
        ...
        q = static_cast<int>(x); r = static_cast<int>(std::floor(numer - (x * denom)));
    } else {
        double x = std::floor(-numer / denom);
        q = -static_cast<int>(x); r = static_cast<int>(std::floor(-numer - (x * denom)));
        if (r != 0) { q--; r = static_cast<int>(denom) - r; }
    }
    return {q, r};
}

int GreatestCommonDivisor(int i1, int i2) { return std::gcd(i1, i2); }

} // namespace Math

//=============================================================================
// 8. System/SDL Abstraction & main
//=============================================================================
...
void Sys_Printf(const char* fmt, ...) {
    ...
    va_start(argptr, fmt); vsprintf_s(text, sizeof(text), fmt, argptr); va_end(argptr);
    std::fprintf(stderr, "%s", text);
}

void Sys_Quit(void) { Host_Shutdown(); std::exit(0); }
void Sys_Init(void) {}
...
static int Qfilelength(FILE* f) {
    long pos = std::ftell(f); std::fseek(f, 0, SEEK_END);
    long end = std::ftell(f); std::fseek(f, pos, SEEK_SET);
    return static_cast<int>(end);
}

int Sys_FileOpenRead(const char* path, int* hndl) {
    FILE* f = nullptr; int i = findhandle();
    if (fopen_s(&f, path, "rb") != 0 || !f) { *hndl = -1; return -1; }
    sys_handles[static_cast<size_t>(i)] = f; *hndl = i;
...
void Sys_FileClose(int handle) {
    if (FILE* f = get_file_handle(handle)) { std::fclose(f); sys_handles[static_cast<size_t>(handle)] = nullptr; }
}
...
int Sys_FileRead(int handle, void* dst, int count) {
    ...
    if (!f) return 0;
    auto* data = static_cast<char*>(dst); int size = 0;
    while (count > 0) {
        int done = static_cast<int>(std::fread(data, 1, count, f));
        if (done == 0) break;
        data += done; count -= done; size += done;
    }
    return size;
}
...
int Sys_FileWrite(int handle, const void* src, int count) {
    FILE* f = get_file_handle(handle);
    if (!f) return 0;
    auto* data = static_cast<const char*>(src); int size = 0;
    while (count > 0) {
...
int Sys_FileTime(const char* path) {
    FILE* f = nullptr;
    if (fopen_s(&f, path, "rb") == 0 && f) { std::fclose(f); return 1; }
    return -1;
}
...
int main(int c, char** v) {
    quakeparms_t parms;
    extern int vcrFile; extern qboolean recording; static int frame = 0;
    moncontrol(); std::signal(SIGFPE, SIG_IGN);

    parms.memsize = 64 * 1024 * 1024; parms.membase = std::malloc(parms.memsize);
    parms.basedir = basedir; parms.cachedir = cachedir;

    COM_InitArgv(c, v); parms.argc = com_argc; parms.argv = com_argv;
    Sys_Init(); Host_Init(&parms); Cvar::Register(&sys_nostdout);

    double oldtime = Sys_FloatTime() - 0.1;
    while (1) {
        double newtime = Sys_FloatTime(); double time = newtime - oldtime;
        if (cls.state == ca_dedicated) {
            if (time < sys_ticrate.value && (vcrFile == -1 || recording)) {
                SDL_Delay(1); continue;
            }
            time = sys_ticrate.value;
        }
        if (time > sys_ticrate.value * 2) oldtime = newtime;
        else oldtime += time;
        if (++frame > 10) moncontrol();
        Host_Frame(static_cast<float>(time)); moncontrol();
    }
}
...
void W_CleanupName(eastl::string_view in, eastl::span<char, 16> out) {
    ...
    for (; i < len && in[i] != '\0'; ++i) out[i] = static_cast<char>(std::tolower(static_cast<unsigned char>(in[i])));
    for (; i < 16; ++i) out[i] = '\0';
}
...
void W_LoadWadFile(eastl::string_view filename) {
    ...
    if (!wad_base) Sys_Error("W_LoadWadFile: couldn't load %s", fname.c_str());
    ...
    wad_lumps = reinterpret_cast<lumpinfo_t*>(wad_base + LittleLong(header->infotableofs));
    lumpinfo_t* lump_p = wad_lumps;
    for (int i = 0; i < wad_numlumps; ++i, ++lump_p) {
        lump_p->filepos = LittleLong(lump_p->filepos); lump_p->size = LittleLong(lump_p->size);
...
void SwapPic(qpic_t* pic) {
    if (pic != nullptr) { pic->width = LittleLong(pic->width); pic->height = LittleLong(pic->height); }
}
