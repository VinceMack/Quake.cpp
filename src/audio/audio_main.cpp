...
void S_SoundInfo_f()
{
    ...
    Console::Con_Printf("%5d stereo\n%5d samples\n%5d samplepos\n%5d samplebits\n%5d submission_chunk\n%5d speed\n0x%x "
        ...
        shm->submission_chunk.load(), shm->speed.load(), shm->buffer.load(),
...
void S_Init()
{
    ...
    for (auto* c : { &nosound, &volume, &precache, &loadas8bit, &bgmvolume, &bgmbuffer, &ambient_level, &ambient_fade,
             &snd_noextraupdate, &snd_show, &_snd_mixahead }) {
        Cvar::Register(c);
    }
...
sfx_t* S_FindName(std::string_view name)
{
    ...
    auto it = std::find_if(
        known_sfx.begin(), known_sfx.end(), [name](const sfx_t& s) { return std::string_view(s.name) == name; });
    ...
    int entnum, int entchannel, sfx_t* sfx, const Vector3& origin, float fvol, float attenuation, int random_offset)
{
    ...
    *target = { .sfx = nullptr,
        ...
        .master_vol = static_cast<int>(fvol * 255) };
...
void S_StopSoundInternal(int entnum, int entchannel)
{
    ...
    if (auto it = std::find_if(active.begin(), active.end(),
            [=](const channel_t& c) { return c.entnum == entnum && c.entchannel == entchannel; });
        it != active.end()) {
        *it = { };
    }
}
...
void S_StaticSoundInternal(sfx_t* sfx, const Vector3& origin, float vol, float attenuation)
{
    ...
    ss = { .sfx = sfx,
        ...
        .master_vol = static_cast<int>(vol) };
    SND_Spatialize(&ss);
}
...
void ExecuteAudioCommand(const AudioCommand& cmd)
{
    ...
    case AudioCommandType::StartSound:
        S_StartSoundInternal(
            cmd.entnum, cmd.entchannel, cmd.sfx, cmd.origin, cmd.vol, cmd.attenuation, cmd.random_offset);
        break;
...
void S_StartSound(int entnum, int entchannel, sfx_t* sfx, const Vector3& origin, float fvol, float attenuation)
{
    ...
    PushAudioCommand({ .type = AudioCommandType::StartSound,
        .entnum = entnum,
        .entchannel = entchannel,
        .sfx = sfx,
        .origin = origin,
...
{
    if (sound_started)
        PushAudioCommand({ .type = AudioCommandType::StopSound, .entnum = entnum, .entchannel = entchannel });
}
    ...
    bool snd_ambient_val)
{
    ...
    for (int i = 0; i < NUM_AMBIENTS; ++i) {
        ...
        chan.master_vol = (chan.master_vol < target) ? std::min(target, chan.master_vol + delta)
                                                     : std::max(target, chan.master_vol - delta);
    ...
    for (size_t idx = 0; idx < active_chans.size(); ++idx) {
        ...
        if (int channel_index = NUM_AMBIENTS + static_cast<int>(idx); channel_index >= static_start) {
            ...
            if (auto match = std::find_if(
                    static_span.begin(), static_span.end(), [&ch](const channel_t& o) { return o.sfx == ch.sfx; });
                match != static_span.end()) {
    ...
    if (fakedma && snd_show.value) {
        Console::Con_Printf("----(%i)----\n",
            static_cast<int>(std::count_if(channels.begin(), channels.begin() + total_channels,
                [](const channel_t& c) { return c.sfx && (c.leftvol || c.rightvol); })));
    }
}
...
void S_Update(const Vector3& origin, const Vector3& forward, const Vector3& right, const Vector3& up)
{
    ...
    AudioCommand cmd { .type = AudioCommandType::ListenerUpdate,
        .origin = origin,
        .vol = volume.value,
        .v_forward = forward,
        .v_right = right,
        .v_up = up,
        .host_frametime = static_cast<float>(Host::host_frametime),
        .ambient_fade = ambient_fade.value,
