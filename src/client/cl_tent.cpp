...
void CL_ParseTEnt()
{
    ...
    switch (type) {
    case TE_WIZSPIKE:
        PosSnd(cl_sfx_wizhit, 20, 30);
        break;
    case TE_KNIGHTSPIKE:
        PosSnd(cl_sfx_knighthit, 226, 20);
        break;
    case TE_SPIKE:
    case TE_SUPERSPIKE: {
        Vector3 pos = PosSnd(nullptr, 0, (type == TE_SPIKE) ? 10 : 20);
        Audio::sfx_t* s = (rand() % 5)
            ? cl_sfx_tink1
            : (std::array { cl_sfx_ric3, cl_sfx_ric1, cl_sfx_ric2, cl_sfx_ric3 }[rand() & 3]);
    ...
    case TE_TAREXPLOSION:
        Render::R_BlobExplosion(PosSnd(cl_sfx_r_exp3));
        break;
    case TE_LIGHTNING1:
    ...
    case TE_BEAM:
        ...
        break;
    case TE_LAVASPLASH:
        Render::R_LavaSplash(PosSnd(nullptr));
        break;
    case TE_TELEPORT:
        Render::R_TeleportSplash(PosSnd(nullptr));
        break;
    default:
        Common::Sys_Error("CL_ParseTEnt: bad type");
    }
}
    ...
    for (auto& b : cl_beams) {
        ...
        if (dist.y == 0.0f && dist.x == 0.0f) {
            ...
            pitch = static_cast<float>(
                std::atan2(dist.z, std::sqrt(dist.x * dist.x + dist.y * dist.y)) * 180.0 / std::numbers::pi);
