...
constexpr keyname_t keynames[] = { { "TAB", K_TAB }, { "ENTER", K_ENTER }, { "ESCAPE", K_ESCAPE }, { "SPACE", K_SPACE },
    { "BACKSPACE", K_BACKSPACE }, { "UPARROW", K_UPARROW }, { "DOWNARROW", K_DOWNARROW }, { "LEFTARROW", K_LEFTARROW },
    { "RIGHTARROW", K_RIGHTARROW }, { "ALT", K_ALT }, { "CTRL", K_CTRL }, { "SHIFT", K_SHIFT }, { "F1", K_F1 },
    { "F2", K_F2 }, { "F3", K_F3 }, { "F4", K_F4 }, { "F5", K_F5 }, { "F6", K_F6 }, { "F7", K_F7 }, { "F8", K_F8 },
    { "F9", K_F9 }, { "F10", K_F10 }, { "F11", K_F11 }, { "F12", K_F12 }, { "INS", K_INS }, { "DEL", K_DEL },
    { "PGDN", K_PGDN }, { "PGUP", K_PGUP }, { "HOME", K_HOME }, { "END", K_END }, { "MOUSE1", K_MOUSE1 },
    { "MOUSE2", K_MOUSE2 }, { "MOUSE3", K_MOUSE3 }, { "JOY1", K_JOY1 }, { "JOY2", K_JOY2 }, { "JOY3", K_JOY3 },
    { "JOY4", K_JOY4 }, { "AUX1", K_AUX1 }, { "AUX2", K_AUX1 + 1 }, { "AUX3", K_AUX1 + 2 }, { "AUX4", K_AUX1 + 3 },
    ...
    { "PAUSE", K_PAUSE }, { "MWHEELUP", K_MWHEELUP }, { "MWHEELDOWN", K_MWHEELDOWN }, { "SEMICOLON", ';' },
    { nullptr, 0 } };
...
void Key_Console(int key)
{
    ...
    if (key == K_UPARROW) {
        ...
        key_linepos = Common::Q_strlen(key_lines[edit_line].data());
        return;
    }
    if (key == K_DOWNARROW) {
...
void Key_Message(int key)
{
    ...
    if (key >= 32 && key <= 127 && chat_bufferlen < 31) {
...
int Key_StringToKeynum(std::string_view str)
{
    ...
    const auto it = std::find_if(std::begin(keynames), std::end(keynames),
        [str](const keyname_t& kn) { return kn.name && Common::Q_strcasecmp(str, kn.name) == 0; });
...
const char* Key_KeynumToString(int keynum)
{
    ...
    const auto it = std::find_if(std::begin(keynames), std::end(keynames),
        [keynum](const keyname_t& kn) { return kn.name && kn.keynum == keynum; });
    return (it != std::end(keynames)) ? it->name : "<UNKNOWN KEYNUM>";
}

void Key_SetBinding(int keynum, const char* binding)
{
    if (keynum >= 0 && keynum < 256) keybindings[keynum] = binding;
}
...
void Key_Unbind_f()
{
    ...
    if (b == -1)
        Console::Con_Printf("\"%s\" isn't a valid key\n", Cmd::Argv(1));
    else
        Key_SetBinding(b, "");
}
...
{
    for (auto& kb : keybindings) kb.clear();
}
...
void Key_Bind_f()
{
    ...
    if (c == 2) {
        if (!keybindings[b].empty())
            ...
            Console::Con_Printf("\"%s\" is not bound\n", Cmd::Argv(1));
        return;
    }
    char cmd[1024] = "";
    for (int i = 2; i < c; i++) {
        if (i > 2) Common::Q_strcat(cmd, " ");
        Common::Q_strcat(cmd, Cmd::Argv(i));
    }
...
void Key_Init()
{
    ...
    for (int k : { K_ENTER, K_TAB, K_LEFTARROW, K_RIGHTARROW, K_UPARROW, K_DOWNARROW, K_BACKSPACE, K_PGUP, K_PGDN,
             K_SHIFT, K_MWHEELUP, K_MWHEELDOWN })
        consolekeys[k] = true;
    ...
    for (int i = 0; i < 12; i++) menubound[K_F1 + i] = true;
...
void Key_Event(int key, bool down)
{
    ...
    if (down) {
        ...
        if (key >= 200 && keybindings[key].empty())
            Console::Con_Printf("%s is unbound, hit F4 to set.\n", Key_KeynumToString(key));
    }
    ...
    if (key == K_ESCAPE) {
        ...
        case key_console:
            ...
            break;
        default:
            Common::Sys_Error("Bad key_dest");
        }
        return;
    }
    if (!down) {
        auto ExecRelease = [](int k) {
            const auto& kb = keybindings[k];
            if (!kb.empty() && kb[0] == '+') Cmd::BufferAddText(Common::va("-%s %i\n", kb.c_str() + 1, k));
        };
        ExecRelease(key);
        if (keyshift[key] != key) ExecRelease(keyshift[key]);
        return;
    }
    if (Client::cls.demoplayback && down && consolekeys[key] && key_dest == key_game) {
        Menu::M_ToggleMenu_f();
        return;
    }
    if ((key_dest == key_menu && menubound[key]) || (key_dest == key_console && !consolekeys[key])
        ...
        if (!kb.empty()) {
            if (kb[0] == '+')
                Cmd::BufferAddText(Common::va("%s %i\n", kb.c_str(), key));
            else {
...
void IN_Move(usercmd_t* cmd)
{
    ...
    if ((Client::in_strafe.state & 1)
        || (Client::lookstrafe.value && ((Client::in_mlook.state & 1) || _windowed_mouse.value))) {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYUP:
            ...
            case SDLK_UP:
                ...
                break;
            case SDLK_DOWN:
                sym = Keys::K_DOWNARROW;
                break;
            case SDLK_RIGHT:
                sym = Keys::K_RIGHTARROW;
                break;
            case SDLK_LEFT:
                sym = Keys::K_LEFTARROW;
                break;
            case SDLK_INSERT:
                sym = Keys::K_INS;
                break;
            ...
            case SDLK_LALT:
                ...
                break;
            case SDLK_KP_0:
                sym = (modstate & KMOD_NUM) ? SDLK_0 : Keys::K_INS;
                break;
            case SDLK_KP_1:
                sym = (modstate & KMOD_NUM) ? SDLK_1 : Keys::K_END;
                break;
            case SDLK_KP_2:
                sym = (modstate & KMOD_NUM) ? SDLK_2 : Keys::K_DOWNARROW;
                break;
            case SDLK_KP_3:
                sym = (modstate & KMOD_NUM) ? SDLK_3 : Keys::K_PGDN;
                break;
            case SDLK_KP_4:
                sym = (modstate & KMOD_NUM) ? SDLK_4 : Keys::K_LEFTARROW;
                break;
            case SDLK_KP_5:
                sym = SDLK_5;
                break;
            case SDLK_KP_6:
                sym = (modstate & KMOD_NUM) ? SDLK_6 : Keys::K_RIGHTARROW;
                break;
            case SDLK_KP_7:
                sym = (modstate & KMOD_NUM) ? SDLK_7 : Keys::K_HOME;
                break;
            case SDLK_KP_8:
                sym = (modstate & KMOD_NUM) ? SDLK_8 : Keys::K_UPARROW;
                break;
            case SDLK_KP_9:
                sym = (modstate & KMOD_NUM) ? SDLK_9 : Keys::K_PGUP;
                break;
            }
            if (sym > 255) sym = 0;
        ...
        case SDL_MOUSEMOTION:
            if (((unsigned)event.motion.x != (Vid::vid.width / 2))
                || ((unsigned)event.motion.y != (Vid::vid.height / 2))) {
                Input::IN_MouseMove(
                    static_cast<float>(event.motion.xrel * 10), static_cast<float>(event.motion.yrel * 10));
                if (((unsigned)event.motion.x < ((Vid::vid.width / 2) - (Vid::vid.width / 4)))
                    || ((unsigned)event.motion.x > ((Vid::vid.width / 2) + (Vid::vid.width / 4)))
                    || ((unsigned)event.motion.y < ((Vid::vid.height / 2) - (Vid::vid.height / 4)))
                    || ((unsigned)event.motion.y > ((Vid::vid.height / 2) + (Vid::vid.height / 4)))) {
                    if (Vid::GetWindow())
                        SDL_WarpMouseInWindow(Vid::GetWindow(), Vid::vid.width / 2, Vid::vid.height / 2);
                }
            }
            break;
        case SDL_QUIT:
